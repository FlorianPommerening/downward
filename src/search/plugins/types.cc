...
FeatureType::FeatureType(type_index pointer_type, const string &type_name,
                         const string &fully_qualified_type_name,
                         const string &synopsis, bool supports_variable_binding)
    : pointer_type(pointer_type), type_name(type_name),
      fully_qualified_type_name(fully_qualified_type_name), synopsis(synopsis),
...
EnumType::EnumType(type_index type, const string&fully_qualified_type_name,
                   const EnumInfo &documented_values)
    : type(type), fully_qualified_type_name(fully_qualified_type_name),
...
const FeatureType &TypeRegistry::create_feature_type(const CategoryPlugin &plugin) {
    ...
    if (registered_types.count(type)) {
        ABORT("Creating the FeatureType '" + plugin.get_pointer_class_name()
...
const EnumType &TypeRegistry::create_enum_type(const EnumPlugin &plugin) {
    ...
    unique_ptr<EnumType> type_ptr = make_unique<EnumType>(
        type, fully_qualified_type_name, values);
